...
- Azure repo upload helper now supports explicit `--auto-detect` org/project discovery when org/project are omitted.
...
- Keep the Azure repo upload helper's `--auto-detect` behavior aligned with Azure CLI defaults and existing Azure remotes when no org/project values are passed.
